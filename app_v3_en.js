...
function append_files_to_list(path, files) {
  ...
  for (i in files) {
    ...
    if (item.mimeType == "application/vnd.google-apps.folder") {
      ...
      html += `<li class="mdui-list-item file mdui-ripple" target="_blank"><form method="post" onSubmit="validate(this);"><input type="checkbox" value="${item.name}" name="track"><a gd-type="${item.mimeType}" href="${p}" class="${c}">
	          <div class="mdui-col-xs-12 mdui-col-sm-7 mdui-text-truncate" title="${item.name}">
